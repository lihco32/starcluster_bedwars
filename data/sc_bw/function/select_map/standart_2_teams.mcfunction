...
execute if score #IS_GAME_RUNNING sc_bw matches 0 run execute at @e[type=marker, tag=sc_bw_lobby_center] run tellraw @a[distance=..30] [{"text":"Бедварс", "color":"#668cff"},{"text":" : "}, {"text":"Обрано карту Стандарт на 2 команди", "color":"white"}]
execute if score #IS_GAME_RUNNING sc_bw matches 0 run execute at @e[type=marker, tag=sc_bw_lobby_center] run tellraw @a[distance=..30] {"text":"================","color":"#0033cc"}

execute if score #IS_GAME_RUNNING sc_bw matches 0 run scoreboard players set #SELECTED_MAP sc_bw 1
